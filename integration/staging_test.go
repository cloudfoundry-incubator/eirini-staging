...
`),
					...
					It("should fail with exit code 222", func() {
						server.AppendHandlers(
							ghttp.CombineHandlers(
								ghttp.VerifyRequest("PUT", responseURL),
								verifyResponse(true, "Failed to detect buildpack: exit status 222"),
							),
						)
						...
						session, err = gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
						Eventually(session, 80).Should(gexec.Exit())
						Expect(session.ExitCode()).To(Equal(eirinistaging.DETECT_FAIL_CODE))
					})
				})
...
`),
					...
					It("should fail with exit code 223", func() {
						server.AppendHandlers(
							ghttp.CombineHandlers(
								ghttp.VerifyRequest("PUT", responseURL),
								verifyResponse(true, "Failed to compile droplet: exit status 223"),
							),
						)
						...
						session, err = gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
						Eventually(session, 80).Should(gexec.Exit())
						Expect(session.ExitCode()).To(Equal(eirinistaging.COMPILE_FAIL_CODE))
					})
				})
...
`),
					...
					It("should fail with exit code 224", func() {
						server.AppendHandlers(
							ghttp.CombineHandlers(
								ghttp.VerifyRequest("PUT", responseURL),
								verifyResponse(true, "Failed to build droplet release: exit status 224"),
							),
						)
						...
						session, err = gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
						Eventually(session, 80).Should(gexec.Exit())
						Expect(session.ExitCode()).To(Equal(eirinistaging.RELEASE_FAIL_CODE))
					})
				})
